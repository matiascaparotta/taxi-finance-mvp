...
test("guarda una sola liquidación compartida por conductor y mes", () => {
  assert.match(migration, /CREATE TABLE IF NOT EXISTS monthly_settlements/);
  assert.match(
    migration,
...
test("protege el cierre definitivo y conserva una auditoría", () => {
  assert.match(migration, /closed_snapshot JSON/);
  assert.match(migration, /closed_by_user_id INT/);
  assert.match(
    migration,
